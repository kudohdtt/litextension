...
class LeCartCustom(LeBasecart):
	def display_config_source(self):
		...
		self._notice['src']['support']['taxes'] = False
	...
	def convert_category_export(self, category, categories_ext):
		...
		# img = get_row_from_list_by_field(categories_ext['data']['cate_imgs'],'categories_id', category['categories_id'])
		...
		category_data['created_at'] = datetime.fromtimestamp(to_int(category['date_added'])).isoformat() if category['date_added'] else '0000-00-00 00:00:00'
		category_data['updated_at'] = datetime.fromtimestamp(to_int(category['last_modified'])).isoformat() if category['last_modified'] else '0000-00-00 00:00:00'
		category_description = get_row_from_list_by_field(categories_ext['data']['categories_descriptions'],'categories_id', category['categories_id'])
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			# 'products_meta': {
			# 	'type': "select",
			# 	'query': "SELECT * FROM  _DBPRF_METAProducts WHERE products_id IN  " + product_id_con,
			# },
			# 'URIs': {
			# 	'type': "select",
			# 	'query': "SELECT * FROM URIs WHERE products_id IN " + product_id_con,
			# },
			# 'specials': {
			# 	'type': "select",
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['created_at'] = datetime.fromtimestamp(to_int(product['products_date_added'])).isoformat() if product['products_date_added'] else get_current_time().isoformat()
		product_data['updated_at'] = datetime.fromtimestamp(to_int(product['products_last_modified'])).isoformat() if to_int(product['products_last_modified']) else product['products_date_added']
		...
		# product_data['tags'] = product_description_def['tag']
		url_product_image = 'http://192.168.68.187:8282//customcart/images/'
		...
		# product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'products_id',product['id'])
		# if product_images:
		# 	if check_thumbnail:
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		if address_books:
			for address_book in address_books:
				...
				if country:
					address_data['country']['id'] = country['entry_country_id']
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			...
			'orders_customer': {
				'type': 'select',
				'query': "SELECT * FROM ` _DBPRF_customers` WHERE customers_id IN " + self.list_to_in_condition(customer_ids)
			},
			# 'orders_payment': {
			# 	'type': 'select',
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_customers = get_row_from_list_by_field(orders_ext['data']['orders_customer'], 'customers_id', order[
			'client_customers_id'])
		...
		order_data['total']['amount'] = order['order_total']
		order_data['subtotal']['title'] = 'Total products'
		order_data['subtotal']['amount'] = order['order_total']
	
		# order_data['created_at'] = datetime.fromtimestamp(to_int(get_value_by_key_in_dict(order, 'orders_date_finished', 0))).strftime('%Y-%m-%d %H:%M:%S')
		...
		if order_customers :
			order_customer['id'] = order_customers['customers_id']
			order_customer['email'] = order_customers['customers_email_address']
			order_customer['first_name'] = order_customers['customers_firstname']
			order_customer['last_name'] = order_customers['customers_lastname']
		...
		order_address = self.construct_order_address()
		# customer_address = self.addConstructDefault(customer_address)
		order_address['first_name'] = order['delivery_name']
		# customer_address['last_name'] = order['ship_lname']
		order_address['address_1'] = order['delivery_address1']
		order_address['address_2'] = order['delivery_address2']
		order_address['city'] = order['delivery_city']
		order_address['postcode'] = order['delivery_postcode']
		# customer_address['telephone'] = order['ship_phone']
		order_address['company'] = order['delivery_company']
		order_address['state']['name'] =  order['delivery_state']
		order_address['country']['nam'] = order['delivery_country']
		order_data['customer_address'] = order_address
		order_data['shipping_address'] = order_address
		...
		if order_customers :
			order_billing['first_name'] = order_customers['customers_firstname']
			# order_billing['first_name'] = order['bill_fname']
			order_billing['last_name'] = order_customers['customers_lastname']
	...
	def product_to_in_condition_seourl(self, ids):
		if not ids:
			return "('null')"
		result = "('products_id=" + "','products_id=".join([str(id) for id in ids]) + "')"
		return result

	def category_to_in_condition_seourl(self, ids):
		if not ids:
			return "('null')"
		result = "('categorys_id=" + "','categorys_id=".join([str(id) for id in ids]) + "')"
		return result
